...
contract RefContract {
    ...
    modifier onlyAdmin() {
        require(
            (msg.sender == admin_contract),
            "The function is reserved for admin"
        );
        _;
    }
    bool private locked;
    ...
    function handleDownReferals(address _user) private {
        for (uint256 i = 0; i < 6; i++) teamUsers[_user].downReferrals.push();
    ...
    function stake(
        ...
        teamUsers[_user].stakes.push(
            Stake(block.timestamp, block.timestamp + 60 * 200, _amount)
        );
    }
        ...
        while (friend != address(0)) {
            if (teamUsers[friend].rank > tempRank) {
                teamUsers[friend].rankBonus.push(
                    RankBonus(
                        block.timestamp,
                        block.timestamp + 200 * 60,
                        teamUsers[friend].rank * 3,
                        reward,
                        referer
                    )
                );
                ...
                sameRank &&
                teamUsers[friend].rank != 0 &&
                teamUsers[friend].rank == tempRank
            ) {
    ...
    function upgradeUser(address _user) public onlyAdmin nonReentrant {
    ...
    function reduceRankBonus(address _user, address referer) private {
        ...
        for (uint256 i = 0; i < rankBonus.length; i++)
            if (
                rankBonus[i].end > currentTime &&
                referer == rankBonus[i].referer
            ) {
                teamUsers[_user].rankBonus[i].end = currentTime;
            }
    }
    ...
    function getValidStakes(
        ...
        for (uint256 i = 0; i < teamUsers[_user].downReferrals[0].length; i++) {
            ...
            for (uint j = 0; j < stakes.length; j++) {
                if (stakes[j].end > currentTime) {
                    teamUsers[intitiator].rankBonus.push(
                        RankBonus(
                            currentTime,
                            stakes[j].end,
                            multiplier,
    ...
    function pushUp(address _user) private {
        ...
        if (
            referer != address(0) &&
            teamUsers[referer].rank == teamUsers[_user].rank
        ) {
            Stake[] memory stakes = teamUsers[_user].stakes;
            for (uint i = 0; i < stakes.length; i++) {
                if (stakes[i].end > currentTime) {
                    teamUsers[referer].rankBonus.push(
                        RankBonus(
                            currentTime,
                            stakes[i].end,
                            multiplier,
                            stakes[i].amount / 10000,
                            _user
                        )
                    );
                }
